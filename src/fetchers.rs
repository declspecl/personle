...
{
    let megaten_persona_data_by_name: HashMap<String, MegatenFusionToolPersonaData> = serde_json::from_str(
...
{
    let special_recipes: HashMap<String, Vec<String>> = serde_json::from_str(
        &reqwest::get(special_recipes_url)
            .await?
            .text()
            .await?
    )?;
