...
const STAR_PRIMARY_COLOR = "#000000",
	STAR_SECONDARY_COLOR = "#666666";
const STAR_MIN_RADIUS = 10,
	STAR_MAX_RADIUS = 18;
const STAR_MIN_LAYERS = 5,
	STAR_MAX_LAYERS = 8;
const STAR_MIN_ROTATION_RADIANS = 0,
	STAR_MAX_ROTATION_RADIANS = Math.PI * 2;
...
export function StarBackground({ className, ...props }: StarBackgroundProps) {
	...
	const drawNestedStars = useCallback(
		(ctx: CanvasRenderingContext2D, x: number, y: number, radius: number, rotation: number, colors: string[], layers: number): void => {
		...
		requestAnimationFrame(() => {
			...
			for (const [i, point] of points.entries()) {
				...
				drawNestedStars(
					...
					radius,
					rotation,
					isPrimaryInner ? [STAR_BACKGROUND_COLOR, primaryColor] : [primaryColor, STAR_BACKGROUND_COLOR],
					layers
				);
			}
		});
	...
	return <canvas ref={canvasRef} width={windowDimensions.width} height={windowDimensions.height} className={className} {...props} />;
}
